...
小红书 MCP 是低频补充数据源，不作为阻塞式依赖。禁止并发搜索、禁止大量翻页、禁止连续重试；超时后降级，继续完成调研并标注待补查。
...
3. 景点活动：先用高德 POI/详情定位，再按 `references/02-attractions-activities.md` 低频补门票、避雷和图片。
4. 酒店餐饮：先用高德周边搜索围绕景区簇筛选位置，再按 `references/03-hotels-dining.md` 低频补价格、评价、链接。
...
- 小红书超时：记录失败关键词、重试情况和“待补查”维度，不得中断全流程。
